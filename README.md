...
# SpectralSignet — 超球面谱几何 · AI 模型身份认证与家族溯源引擎

## 标签（Keywords）

`llm` · `spectral-analysis` 谱分析 · `geometric-fingerprint` 几何指纹 · `model-traceability` 模型溯源 · `family-lineage` 家族谱系 · `aiq-health-diagnosis` 健康诊断 · `anti-finetuning-robustness` 抗微调鲁棒性 · `trustworthy-ai` 可信AI

## 产品介绍

SpectralSignet 是面向大语言模型的"几何指纹"身份认证与溯源系统。其核心思想源于一个深刻的几何观测：任何经过训练的前馈网络，其激活分布在超球面谱空间中都会留下稳定的几何印记——Gegenbauer 谱投影的能量占比剖面（β/Γ 剖面）如同生物的指纹，具有**家族级可分性、实例级唯一性、对微调（SFT）的强鲁棒性**。

系统以数学物理为根基，把模型隐空间的几何分解为可量化、可复现的谱特征：

- **谱指纹（Spectral Signet）**：经 C-子空间主成分分析（M=3）提取 Gamma 谱集中度剖面，回答"模型把表征能量寄托在哪些子空间"；
- **健康画像（AIQ）**：融合 Gamma、曲率分布（K%/DEFF/Hmed）与谱集中度等真实实测，输出 AIQ 综合智能商与退化/过深/短链等结构性诊断；
- **家族溯源（Lineage Tracing）**：基于剖面 MAD 距离与凝聚聚类实现跨层数模型家族归属判定——Qwen base↔Instruct 距离 0.0031 精准同族，跨家族显著分离；
- **抗微调鲁棒性（Robustness）**：SFT/截断/改写三类扰动下指纹存活率 100%，为版权维权与模型来源审计提供证据级支撑。

工程上，SpectralSignet 以 59 参数验证体系为底座，采用"优化工厂"四层架构（ConfigFactory / FingerprintSynthesizer / ValidatorEngine / ReportGenerator），pydantic 类型安全、结构化 JSON 观测、向量化性能优化，覆盖从真实模型测量、健康诊断到可视化报告的完整闭环，支持 HTML/JSON 报告导出与 CI 门禁（覆盖率 ≥95%、59 项冒烟全绿）。
